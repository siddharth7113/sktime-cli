...
the same model the Hugging Face CLI uses:
...
- **Results go to stdout. Everything else, meaning logs, warnings, errors,
  and result counts, goes to stderr.** You can always pipe stdout.
...
codes, and the table is **append-only**, so codes are never renamed:
...
installed). The CLI runs it once and serializes every record, meaning name, module,
scitypes, tags, parameters, and installability, to JSON in the workspace.
...
- JSON uses pandas **split orient** (`{"index", "columns", "data"}`) in both
  directions, which is the one JSON shape that round-trips indexes losslessly.
...
That is useful for humans, and the designed first move for agents handed an unknown
file.
...
- **Derive, don't duplicate**: configuration comes from `pyproject.toml` and
  `importlib.metadata` where possible. For example, `doctor`'s optional-dependency
...
- Typer + rich for the frontend; the console script is `sktime-cli` only,
  with no `sktime` short name that could collide with the library.
...
- See the [Roadmap](../roadmap.md) for the full deferred list, and
  [Working around upstream sktime](sktime-notes.md) for the upstream
  behaviors the implementation works around.
